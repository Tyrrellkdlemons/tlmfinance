...
/* True PDF download via jsPDF (loaded on demand from esm.sh). Calls
   doc.save() which the browser treats as an explicit file download —
   on desktop it lands in Downloads, on mobile it triggers the device's
   native download/share flow. No print preview. */
async function downloadStarterPlanPDF(tasks, needs /*, _autoPrint ignored */) {
  ...
  try {
    ...
    // Offline / blocked — give the user a plain-text fallback so they always
    // walk away with a file rather than a broken button.
    const lines = [
      'The Last Mile · Finance',
      'Your starter 72-hour plan',
      today, '',
      checked ? ('Focused on: ' + checked) : '', '',
      'Action steps:'
    ].concat((tasks || []).map((t, i) => (i + 1) + '. ' + safeText(t)));
    lines.push('', 'Educational information, not legal or financial advice.', 'tlmfinance.netlify.app');
    ...
    toast('PDF library offline — saved as text instead.');
    return;
  }
  ...
  list.forEach((t, i) => {
    ...
    const wrapped = doc.splitTextToSize(num + safeText(t), W - 2 * M - 6);
    if (y + wrapped.length * 14 + 20 > H - M) { doc.addPage(); y = M; }
...
// (Old print-window implementation kept below behind a feature flag for
// rollback; use the async function above by default.)
function downloadStarterPlanPDF__legacy(tasks, needs, autoPrint = true) {
